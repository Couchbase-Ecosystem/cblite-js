...
export class QueryTests extends TestCase {
    ...
    async testFTSWithFTSIndexDefaultCol(): Promise<ITestResult> {
        try {
            ...
            const indexes: string[] = ["index", "_.index", "_default.index", "d.index"];
            const froms: string[] = ["_", "_", "_default", `\`${this.database.getName()}\``];
    ...
    async testFTSWithFTSIndexNamedCol(): Promise<ITestResult> {
        try {
            ...
            //add documents
            await col.save(new MutableDocument('person1', null, {
                name: {first: 'Jasper', last: 'Grebel'},
                random: '4'
            }));
            await col.save(new MutableDocument('person2', null, {
                name: {first: 'Jasper', last: 'Okorududu'},
                random: '1'
            }));
            await col.save(new MutableDocument('person3', null, {
    ...
    async testFtsJoinCollection(): Promise<ITestResult> {
        try {
            ...
            //add documents
            await flowersCol.save(new MutableDocument('c1', null, {name: 'rose', cid: 1, description: 'Red flowers'}));
            await flowersCol.save(new MutableDocument('c2', null, {
                name: 'hydrangea',
                cid: 2,
    ...
    async testSelectAllResultKey(): Promise<ITestResult> {
        try {
            ...
            //add documents
            await flowersCol.save(new MutableDocument('c1', null, {name: 'rose', cid: 1}));
            await this.defaultCollection.save(new MutableDocument('c1', null, {name: 'rose', cid: 1}));
    ...
    async runQueryParameterTest(
        ...
        paraName: string,
        paraValue: any,
        testName: string): Promise<ITestResult> {
        try {
