...
export class CollectionTests extends TestCase {
    ...
    async testGetDefaultScopeDeleteDefaultCol(): Promise<ITestResult> {
        ...
        try {
            ...
            const defaultCollection2 = await this.database?.createCollection(Database.defaultCollectionName, Database.defaultScopeName);
    ...
    async testCreateAndGetColsInDefaultScope(): Promise<ITestResult> {
        try {
            ...
            const colB = await this.database?.createCollection('colB', Database.defaultScopeName);
    ...
    async isErrorCreatingCollection(name: string, scopeName: string): Promise<boolean> {
        try {
