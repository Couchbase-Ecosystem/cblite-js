...
export class DocumentTests extends TestCase {
  ...
  async testGetString(): Promise<ITestResult> {
    try {
      ...
      expect(retrievedDoc.getArray("array")).to.eql(["650-000-0000", "650-000-0001"]);
  ...
  async testSetGetFloatNumbers(): Promise<ITestResult> {
    try {
      // Create and populate the document
      const doc = new MutableDocument("doc1");
      doc.setDouble("number1", 1.00);
      doc.setDouble("number2", 1.49);
      doc.setDouble("number3", 1.50);
      ...
      expect(retrievedDoc.getFloat("number1")).to.equal(1.00);
      expect(retrievedDoc.getDouble("number1")).to.equal(1.00);
      ...
      expect(retrievedDoc.getFloat("number3")).to.equal(1.50);
      expect(retrievedDoc.getDouble("number3")).to.equal(1.50);
  ...
  async testUpdateNestedArray(): Promise<ITestResult> {
    try {
        ...
        // Retrieve and verify the saved document
        let retrievedDoc = await this.defaultCollection.document("doc1");
        expect(retrievedDoc.getData()).to.deep.equal({ "groups": [["a", "b", "c"], [1, 2, 3]] });
        ...
        // Retrieve and verify the updated document
        retrievedDoc = await this.defaultCollection.document("doc1");
        expect(retrievedDoc.getData()).to.deep.equal({ "groups": [["d", "e", "f"], [4, 5, 6]] });
  ...
  async testSetArrayToMultipleKeys(): Promise<ITestResult> {
    try {
      ...
      expect(doc.getArray("mobile")).to.deep.equal(["650-000-0001", "650-000-0002", "650-000-0003"]);
